...
const generateProblem = (type: string): Problem => {
  ...
  if (type === 'AdvocacyCastle') {
    const scenarios = [
      { q: "I can't see board well. What should I do?", a: "move closer", o: ["stay put", "move closer", "close eyes", "go home"] },
      { q: "I forgot glasses. What should I do?", a: "tell teacher", o: ["hide it", "tell teacher", "ignore it", "cry"] },
      { q: "Teacher is talking. Is it joke time?", a: "no", o: ["yes", "no"] },
      { q: "standing [blank] to move closer anytime.", a: "invitation", o: ["order", "invitation", "rule", "ban"] }
    ];
  ...
  if (type === 'ScienceLab') {
    const facts = [
      { q: "Which planet is known as Red Planet?", a: "Mars", o: ["Venus", "Mars", "Jupiter", "Saturn"] },
      { q: "boiling point of water (C)?", a: "100", o: ["0", "50", "100", "200"] },
      { q: "force that pulls objects to Earth?", a: "gravity", o: ["magnetism", "gravity", "friction", "wind"] }
    ];
  ...
  if (type === 'GeoGlobe') {
    const geo = [
      { q: "largest continent?", a: "Asia", o: ["Africa", "Asia", "Europe", "America"] },
      { q: "ocean is the largest?", a: "Pacific", o: ["Atlantic", "Pacific", "Indian", "Arctic"] },
      { q: "capital of the USA?", a: "Washington DC", o: ["New York", "Washington DC", "LA", "Chicago"] }
    ];
  ...
  if (!isVerified) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 sky-bg pixel-font overflow-y-auto">
      <div className="bg-white border-8 border-black p-10 max-w-md text-center text-black my-auto">
        <h2 className="text-xl mb-8 uppercase font-bold text-black">Admin Access</h2>
        ...
        <button onClick={onExit} className="mt-8 text-[8px] text-gray-400 uppercase underline text-center block w-full">Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 sky-bg pixel-font overflow-y-auto">
      <div className="bg-white border-8 border-black p-8 w-full max-w-2xl text-black my-auto">
        ...
        <div className="max-h-[60vh] overflow-y-auto pr-2 text-black">
          {activeTab === 'Stats' && (
            <div className="space-y-6">
              <div><label className="block text-[10px] uppercase font-bold mb-2 text-black">XP: {stats.xp}</label><input type="range" min="0" max="10000" value={stats.xp} onChange={(e) => onUpdateStats({ ...stats, xp: parseInt(e.target.value) })} className="w-full accent-black" /></div>
              <div><label className="block text-[10px] uppercase font-bold mb-2 text-black">Coins: {stats.coins}</label><input type="range" min="0" max="5000" value={stats.coins} onChange={(e) => onUpdateStats({ ...stats, coins: parseInt(e.target.value) })} className="w-full accent-black" /></div>
              <div><label className="block text-[10px] uppercase font-bold mb-2 text-black">Inventory</label><div className="grid grid-cols-2 gap-2 text-black">{['glasses', 'focus'].map(item => (<button key={item} onClick={() => { const newInv = stats.inventory.includes(item) ? stats.inventory.filter((i: string) => i !== item) : [...stats.inventory, item]; onUpdateStats({ ...stats, inventory: newInv }); }} className={`p-4 border-4 border-black uppercase text-[8px] font-bold ${stats.inventory.includes(item) ? 'bg-green-400' : 'bg-gray-100'}`}>{item}</button>))}</div></div>
            </div>
          )}

          {activeTab === 'History' && (
            <div className="space-y-8">
              <div className="bg-blue-50 border-4 border-black p-4 text-black">
                <h3 className="text-[10px] font-bold mb-4 uppercase">Add New Event</h3>
                <div className="grid gap-4">
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {['LEGO', 'Drawing', 'Gaming', 'Outside', 'Reading'].map(fun => (
                      <button key={fun} onClick={() => addManualEntry(fun)} className="bg-white border-2 border-black p-2 text-[6px] uppercase hover:bg-yellow-100 shrink-0 text-black">{fun}</button>
                    ))}
                  </div>
                  <input type="text" placeholder="Or type activity..." value={newEntry.activity} onChange={e => setNewEntry({...newEntry, activity: e.target.value})} className="border-4 border-black p-2 text-[10px] uppercase w-full bg-white text-black" />
                  <div className="flex gap-4 items-center">
                    <input type="number" value={newEntry.mins} onChange={e => setNewEntry({...newEntry, mins: parseInt(e.target.value) || 0})} className="border-4 border-black p-2 text-[10px] w-24 bg-white text-black" />
                    <span className="text-[8px] uppercase text-black">MINS</span>
              ...
              {Object.entries(history).sort((a, b) => new Date(b[0]).getTime() - new Date(a[0]).getTime()).map(([date, data]: any) => (
                <div key={date} className="border-4 border-black p-4 bg-gray-50 text-black">
                  ...
                  {Object.entries(data).map(([act, mins]: any) => (
                    <div key={act} className="flex justify-between items-center mb-2 text-black">
                      ...
                      <div className="flex items-center gap-2"><input type="number" value={Math.round(mins)} onChange={(e) => { const newMins = parseInt(e.target.value) || 0; onUpdateHistory({ ...history, [date]: { ...data, [act]: newMins } }); }} className="w-12 border-2 border-black p-1 text-[8px] text-center bg-white text-black" /><span className="text-[6px]">MIN</span></div>
                    </div>
                  ))}
                </div>
              ))}
          ...
          {activeTab === 'Nuclear' && (
            <div className="text-center p-8 text-black">
              ...
              <button onClick={() => { if (window.confirm("Reset math?")) { localStorage.removeItem(MULTIPLICATION_STATE_KEY); window.location.reload(); } }} className="bg-orange-500 text-white p-4 border-4 border-black uppercase font-bold text-[10px] w-full mt-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">Reset Math Progress</button>
            </div>
          )}
        </div>
      </div>
...
function QuestJournal({ history, onExit }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 sky-bg pixel-font overflow-y-auto text-black">
      <div className="bg-white border-8 border-black p-8 w-full max-w-4xl my-auto text-center text-black">
        <div className="flex justify-between items-center mb-8 text-black text-center"><h2 className="text-xl uppercase font-bold">Quest Journal</h2><button onClick={onExit} className="text-red-600 uppercase text-xs font-bold">[ Close ]</button></div>
          ...
          {[...Array(days)].map((_, i) => {
            ...
            return (
              <div key={i} className={`aspect-square border-2 border-black flex flex-col items-center justify-center relative group ${total > 0 ? 'bg-yellow-100' : 'bg-gray-50'}`}>
                ...
                {total > 0 && <Star size={12} className="text-yellow-500 fill-yellow-500 text-black" />}
                {data && <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-black text-white p-2 text-[6px] uppercase hidden group-hover:block border-2 border-white z-50">{Object.entries(data).map(([a, m]: any) => <div key={a} className="flex justify-between text-white text-[6px]"><span>{a}</span><span>{Math.round(m)}m</span></div>)}<div className="border-t border-white/20 pt-1 mt-1 text-blue-400 font-bold text-center">Total: {Math.round(total)}m</div></div>}
              </div>
            );
          })}
        </div>
...
function MorningPowerUp({ onComplete }: { onComplete: (items: string[]) => void }) {
  const [tasks, setTasks] = useState([
    { id: 'glasses', text: 'Glasses ON', icon: Eye, done: false },
    { id: 'seat', text: 'Move closer', icon: Target, done: false },
    { id: 'focus', text: 'No distractions', icon: Clock, done: false },
    { id: 'humor', text: 'Jokes ready', icon: MessageSquare, done: false },
  ]);
  ...
  return (
    <div className="max-w-2xl mx-auto my-12 bg-white border-8 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] pixel-font relative text-left text-black">
      ...
      <h2 className="text-xl mb-8 uppercase flex items-center gap-4 font-bold text-center text-black"><Heart className="fill-red-600 text-red-600 animate-pulse" /> Morning Buffs</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-black">
        {tasks.map(t => (
          <button key={t.id} onClick={() => setTasks(tasks.map(task => task.id === t.id ? { ...task, done: !task.done } : task))} className={`flex items-center gap-4 p-4 border-4 border-black transition-all ${t.done ? 'bg-green-100' : 'bg-white hover:bg-gray-50'} text-black`}><div className={`w-10 h-10 border-4 border-black flex items-center justify-center shrink-0 ${t.done ? 'bg-green-500' : 'bg-white'} text-black`}><t.icon size={20} className="text-black" /></div><span className={`text-[10px] text-left leading-tight uppercase text-black ${t.done ? 'line-through text-gray-400' : 'font-bold'}`}>{t.text}</span></button>
        ))}
      </div>
      {allDone && <button onClick={() => onComplete(tasks.map(t => t.id))} className="w-full bg-yellow-500 text-white p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] animate-bounce uppercase text-sm border-4 border-black font-bold text-center">Activate Multiplier!</button>}
    </div>
  );
}

function BattleUI({ gameId, problem, feedback, progress, totalProgress, timeLeft, onExit, onAnswer, onStart, stats }: any) {
  ...
  if (isStarting) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sky-bg pixel-font overflow-y-auto text-black">
      <div className="bg-white border-8 border-black p-10 max-w-lg shadow-[20px_20px_0px_0px_rgba(0,0,0,0.3)] my-auto text-center text-black">
        <h2 className="text-4xl mb-6 uppercase font-black italic tracking-tighter text-black text-center">BOSS INCOMING!</h2>
        <div className="mb-8 p-8 bg-gray-100 border-4 border-black flex flex-col items-center gap-4 text-black">
           <div className="animate-bounce p-4 bg-white border-4 border-black">{getBossIcon()}</div>
           <p className="text-red-600 font-bold uppercase text-xs text-center">Danger Level: EXTREME</p>
           {hasFocus && <p className="text-orange-500 text-[8px] uppercase text-center">Focus Active: Timer Slowed!</p>}
        </div>
        <button onClick={() => { setIsStarting(false); onStart?.(); }} className="bg-red-600 text-white p-6 border-4 border-black uppercase animate-bounce w-full font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-xl text-center">ENGAGE BATTLE</button>
      </div>
    </div>
  );
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col ${isFever ? 'bg-yellow-400' : 'sky-bg'} pixel-font overflow-y-auto text-white transition-colors duration-500`}>
      ...
      <div className="p-8 flex justify-between items-start z-20 shrink-0 text-white">
        <div className="text-left text-white"><p className="text-[10px] text-blue-400 mb-2 font-bold uppercase tracking-widest text-left">{gameId.toUpperCase()}</p><div className="flex items-center gap-4 text-white"><p className="flex items-center gap-2 text-sm font-bold text-white"><Trophy size={16} className="text-yellow-400 text-white" /> {progress}/{totalProgress}</p>{combo > 1 && (<div className={`px-2 py-1 text-[10px] font-black italic animate-bounce border-2 border-black ${isFever ? 'bg-red-600 text-white scale-150' : 'bg-yellow-400 text-black'}`}>{isFever ? 'FEVER MODE!' : `${combo}X COMBO!`}</div>)}</div></div>
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center pb-32 text-black min-h-[500px]">
        <div className={`absolute top-20 transition-all duration-300 transform ${feedback === 'correct' ? 'scale-150 opacity-0 -translate-y-20' : feedback === 'wrong' ? 'scale-110 translate-y-4' : 'animate-pulse'}`}>
          <div className={`relative p-8 bg-white border-8 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,0.1)] ${feedback === 'wrong' ? 'bg-red-100' : ''}`}>{getBossIcon()}<div className="absolute -top-12 left-0 w-full h-4 bg-gray-200 border-4 border-black"><div className="h-full bg-red-600 transition-all duration-500" style={{ width: `${Math.max(0, 100 - (progress/totalProgress * 100))}%` }}></div></div>{damageDealt && (<div className="absolute -right-8 top-0 text-red-600 font-black text-xl animate-bounce text-center">-{damageDealt} HP</div>)}</div>
        </div>
        <div className={`mt-40 mb-12 transform transition-all duration-300 ${feedback === 'wrong' ? 'shake scale-110' : ''}`}><div className="bg-white border-8 border-black p-10 rounded-lg shadow-[12px_12px_0px_0px_rgba(0,0,0,0.2)] text-center relative max-w-md mx-auto text-black"><div className="text-3xl md:text-5xl leading-tight uppercase font-black italic tracking-tighter text-black text-center">{problem?.question}</div>{feedback === 'correct' && (<div className="absolute -top-20 left-1/2 -translate-x-1/2 coin-animate"><div className="text-yellow-500 font-black text-2xl drop-shadow-[2px_2px_0px_rgba(0,0,0,1)] text-center text-black">{isFever ? 'ULTRA KILL!' : 'BOOM!'}</div><Coins size={60} className="text-yellow-400 fill-yellow-400 mx-auto" /></div>)}{hasGlasses && feedback === 'idle' && timeLeft < 5 && (<div className="absolute -bottom-10 left-1/2 -translate-x-1/2 bg-green-500 text-white text-[8px] px-2 py-1 border-2 border-black animate-pulse text-center">HINT: {problem?.answer}</div>)}</div></div>
        {problem && problem.options ? (<div className="grid grid-cols-2 gap-4 w-full max-w-xl px-4 text-black">{problem.options.map((opt: string) => (<button key={opt} onClick={() => onAnswer(opt, isFever ? 3 : 1)} disabled={feedback !== 'idle'} className="bg-white border-8 border-black p-6 text-sm uppercase font-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none hover:bg-yellow-400 transition-colors transform hover:scale-105 text-black text-center">{opt}</button>))}</div>) : (<form onSubmit={handleSubmit} className="flex flex-col items-center w-full max-w-sm px-4 text-black"><input ref={inputRef} type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={feedback !== 'idle'} placeholder={isFever ? "OVERDRIVE!" : "ATTACK!"} className="w-full bg-white border-8 border-black p-6 text-3xl text-center shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] focus:outline-none mb-8 placeholder:opacity-30 uppercase font-black italic text-black" /><button type="submit" className={`${isFever ? 'bg-yellow-400' : 'bg-red-600'} text-white px-12 py-6 border-4 border-black uppercase font-black text-2xl w-full shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:opacity-90 active:translate-y-1 active:shadow-none transition-all text-center`}>{isFever ? 'FEVER STRIKE!' : 'STRIKE!'}</button></form>)}
        <div className={`absolute bottom-32 left-8 md:left-20 transition-all duration-300 ${isJumping ? 'mario-jump' : ''} ${feedback === 'correct' ? 'translate-x-20' : ''}`}><div className={`w-16 h-20 bg-red-600 border-4 border-black relative rounded-sm flex items-center justify-center text-white font-bold text-center ${feedback === 'correct' || isFever ? 'scale-125' : ''} ${isFever ? 'animate-pulse shadow-[0_0_30px_rgba(255,255,0,0.8)] bg-yellow-500' : ''}`}><div className="absolute top-0 w-full h-8 bg-red-800"></div><div className="absolute top-4 w-12 h-8 bg-pink-200 rounded-sm"></div><div className="absolute bottom-4 w-full h-8 bg-blue-600"></div><span className="text-white text-[8px] z-10 font-bold uppercase text-white">O</span>{(feedback === 'correct' || isFever) && <Zap className="absolute -right-8 -top-8 text-yellow-400 animate-pulse text-white" size={40} />}{isFever && <Star className="absolute -left-8 -top-8 text-white animate-spin text-white" size={30} />}</div></div>
      </div>
...
function FactFluencyGame({ onExit, onReward, onLogTime, stats }: any) {
  ...
  if (!difficulty) return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 sky-bg pixel-font overflow-y-auto text-black text-center"><div className="bg-white border-8 border-black p-10 max-w-md text-center my-auto text-black"><h2 className="text-xl mb-4 uppercase font-bold text-black text-center">Difficulty</h2><div className="grid gap-4 text-black text-center">{[15, 10, 5].map(t => <button key={t} onClick={() => setDifficulty(t)} className="mario-brick bg-yellow-500 text-white p-4 border-4 border-black uppercase font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center">W 1-{4-t/5} ({t}s)</button>)}</div><button onClick={() => { if (window.confirm("Reset math?")) { localStorage.removeItem(MULTIPLICATION_STATE_KEY); window.location.reload(); } }} className="mt-8 text-[6px] text-red-500 uppercase font-bold block w-full text-center underline">Reset Math</button><button onClick={onExit} className="mt-4 text-[8px] text-gray-400 uppercase font-bold text-center block w-full text-center">Exit</button></div></div>);
...
function FocusTimer({ title, targetMins, onExit, onComplete, onLogTime }: any) {
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 sky-bg pixel-font overflow-y-auto text-black text-center"><div className="bg-white border-8 border-black p-10 max-w-md text-center my-auto text-black"><h2 className="text-xl mb-4 uppercase font-bold text-black text-center text-black">{title}</h2><div className="text-6xl mb-8 font-bold text-black text-center text-black">{formatTime(seconds)}</div><button onClick={() => setIsActive(!isActive)} className={`w-full text-white p-6 border-4 border-black mb-4 font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${isActive ? 'bg-red-500' : 'bg-green-500'} uppercase text-center`}>{isActive ? 'PAUSE' : 'START'}</button><button onClick={() => { onLogTime((Date.now() - startRef.current) / 60000); onExit(); }} className="text-[8px] text-gray-400 uppercase w-full font-bold underline text-center text-black">Exit</button></div></div>);
}

function LexiaOverlay({ onExit, onLog }: any) {
  return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 sky-bg pixel-font overflow-y-auto text-black text-center"><div className="bg-white border-8 border-black p-10 max-w-md text-center my-auto text-black"><h2 className="text-xl mb-4 text-purple-600 uppercase font-bold text-center">LEXIA POWER-UP</h2><button onClick={() => { onLog(); window.open('https://www.lexiacore5.com/', '_blank'); }} className="bg-purple-600 text-white p-6 border-4 border-black uppercase w-full mb-4 font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center">OPEN</button><button onClick={onExit} className="text-[8px] text-gray-400 uppercase font-bold underline text-center text-black">Back</button></div></div>);
}
...
function SpellingBoardGame({ onExit, onReward, onLogTime }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col sky-bg pixel-font overflow-y-auto text-black">
      <div className="p-8 flex justify-between items-center z-20 shrink-0 text-white">
        <div className="text-left text-white"><p className="text-[10px] text-blue-400 mb-2 font-bold uppercase tracking-widest text-left text-white">SPELL QUEST</p><p className="flex items-center gap-2 text-sm text-white font-bold text-white"><Trophy size={16} className="text-yellow-400 text-white" /> {playerPosition}/{totalTiles - 1}</p></div>
        <button onClick={onExit} className="p-4 bg-red-600 border-4 border-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] uppercase text-[10px] font-bold text-center text-white">Exit</button>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center p-4 min-h-[500px] text-black">
        <div className="relative w-full max-w-4xl h-64 flex items-center justify-between px-12 text-black">
          ...
          {Array.from({ length: totalTiles }).map((_, i) => (
            <div key={i} className={`w-12 h-12 border-4 border-black relative z-10 flex items-center justify-center transition-all ${i === totalTiles - 1 ? 'bg-red-500 scale-125' : i <= playerPosition ? 'bg-green-400' : 'bg-white'} ${i === playerPosition ? 'shadow-[0_0_20px_rgba(255,255,255,0.8)]' : ''}`}>
              ...
              {i === playerPosition && (<div className="absolute -top-16 left-1/2 -translate-x-1/2 animate-bounce text-black"><div className="w-10 h-12 bg-red-600 border-4 border-black relative rounded-sm flex items-center justify-center text-white font-bold"><div className="absolute top-0 w-full h-4 bg-red-800"></div><span className="text-[8px] z-10">O</span></div></div>)}
            </div>
          ))}
        </div>
        {!showChallenge && (
          <div className="mt-12 flex flex-col items-center gap-8 text-black">
            <div className={`w-24 h-24 bg-white border-8 border-black flex items-center justify-center text-4xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${isRolling ? 'animate-spin' : ''} text-black`}>{isRolling ? '?' : diceValue || '🎲'}</div>
            <button onClick={startTurn} disabled={isRolling} className="bg-yellow-500 text-white px-12 py-6 border-4 border-black uppercase font-bold text-xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none disabled:opacity-50 text-center text-white">Roll Dice!</button>
          </div>
        )}
      </div>
      {showChallenge && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 overflow-y-auto text-black">
          <div className="bg-white border-8 border-black p-10 w-full max-w-md shadow-[12px_12px_0px_0px_rgba(255,255,255,0.2)] text-center my-auto text-black">
            <div className="flex flex-col items-center gap-6 mb-8 text-black">
              ...
              <button type="button" onClick={() => setVoiceIndex(v => v + 1)} className="text-[6px] text-blue-500 underline uppercase text-center text-black">(Switch Voice)</button>
            </div>
            <form onSubmit={handleAnswer} className="space-y-6 text-black">
              ...
              <button type="submit" disabled={feedback !== 'idle'} className="w-full bg-red-600 text-white p-4 border-4 border-black uppercase font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center text-white text-xl">Submit</button>
            </form>
          </div>
        </div>
      )}
...
function WorldCard({ id, title, world, icon: Icon, color, desc, onSelect, event }: any) { 
  return (
    <button onClick={() => onSelect(id)} className={`mario-card group relative bg-white border-8 border-black p-6 flex flex-col items-center text-center h-full shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all text-black ${event ? 'border-yellow-400 animate-pulse' : ''}`}>
      ...
      <p className="text-[8px] text-gray-500 uppercase leading-relaxed font-bold text-center text-black">{desc}</p>
    </button>
  ); 
}
...
function FunEventsGame({ onExit, onReward, onLogTime }: any) {
  ...
  const handleLog = () => { if (!selected) return; onLogTime(selected, mins); onReward(mins * 5, mins * 2); alert(`Logged ${mins}m of ${selected}!`); onExit(); };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 sky-bg pixel-font overflow-y-auto text-black">
      <div className="bg-white border-8 border-black p-10 w-full max-w-2xl my-auto text-black text-center text-black">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10 text-black">
          {FUN_ACTIVITIES.map(act => (
            <button key={act.name} onClick={() => setSelected(act.name)} className={`p-4 border-4 border-black flex flex-col items-center gap-2 transition-all ${selected === act.name ? 'bg-yellow-400 scale-110' : 'bg-gray-50 hover:bg-gray-100'} text-black`}>
              <act.icon size={24} className={selected === act.name ? 'animate-bounce' : ''} />
              <span className="text-[6px] uppercase font-bold text-black">{act.name}</span>
            </button>
          ))}
        </div>
        {selected && (
          <div className="bg-blue-50 border-4 border-dashed border-black p-6 mb-8 text-black">
            <p className="text-[10px] mb-4 uppercase font-bold text-center text-black">How long?</p>
            <div className="flex justify-center gap-4 mb-6 text-black">
              {[15, 30, 60, 120].map(m => (<button key={m} onClick={() => setMins(m)} className={`p-2 border-2 border-black text-[8px] uppercase font-bold ${mins === m ? 'bg-black text-white' : 'bg-white text-black'}`}>{m}m</button>))}
            </div>
            <button onClick={handleLog} className="bg-pink-500 text-white p-4 border-4 border-black w-full uppercase font-black text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-xl">Log Fun!</button>
          </div>
        )}
        <button onClick={onExit} className="mt-4 text-[8px] text-gray-400 uppercase underline text-center text-black">Back</button>
      </div>
    </div>
  );
}
...
export default function App() {
  const [stats, setStats] = useState<UserStats>(() => { const saved = localStorage.getItem(USER_STATS_KEY); if (saved) try { return JSON.parse(saved); } catch(e) {} return { xp: 0, coins: 0, level: 1, inventory: [] }; });
  const [history, setHistory] = useState<ActivityEntry>(() => { const saved = localStorage.getItem(ACTIVITY_HISTORY_KEY); if (saved) try { return JSON.parse(saved); } catch(e) {} return {}; });
  const [activeGame, setActiveGame] = useState<GameType | null>(null);
  const [multiplier, setMultiplier] = useState(1);
  const [events, setEvents] = useState<GameEvent[]>([]);
  const [lastQuestDate, setLastQuestDate] = useState(() => { const saved = localStorage.getItem(QUEST_LOG_KEY); if (saved) try { return JSON.parse(saved).date; } catch(e) {} return ''; });
  ...
  useEffect(() => {
    const generateEvent = () => {
      ...
      const types: any[] = [{ id: '1', title: 'BOSS INVASION', multiplier: 2, icon: AlertTriangle, type: 'Invasion' }, { id: '2', title: 'DOUBLE COINS', multiplier: 2, icon: Flame, type: 'DoubleCoins' }, { id: '3', title: 'MYSTERY QUEST', multiplier: 1.5, icon: Gift, type: 'Mystery' }];
      setEvents([{ ...types[getRandomInt(0, types.length - 1)], worldId: world }]);
    };
    generateEvent(); const timer = setInterval(generateEvent, 300000); return () => clearInterval(timer);
  }, []);

  useEffect(() => { 
    const newLevel = getLevel(stats.xp);
    if (newLevel > stats.level) { setStats(prev => ({ ...prev, level: newLevel })); setShowLevelUp(true); setTimeout(() => setShowLevelUp(false), 3000); }
  ...
  const addRewards = (xp: number, coins: number) => { const event = events.find(e => e.worldId === activeGame); const m = event ? event.multiplier : 1; setStats(prev => ({ ...prev, xp: prev.xp + (xp * m), coins: prev.coins + (coins * multiplier * m) })); };
  ...
  return (
    <div className="min-h-screen bg-[#5c94fc] pixel-font pb-20 selection:bg-red-600 selection:text-white relative flex flex-col text-black overflow-y-auto">
      {showLevelUp && (<div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none p-4"><div className="bg-yellow-400 border-8 border-black p-12 animate-bounce text-center shadow-[20px_20px_0px_0px_rgba(0,0,0,1)]"><h2 className="text-5xl font-black uppercase text-black italic tracking-tighter mb-4 text-center">LEVEL UP!</h2><p className="text-xl font-bold uppercase text-black text-center text-black">REACHED LEVEL {stats.level}!</p><div className="mt-4 flex justify-center gap-4 text-black"><Star size={40} className="text-white fill-white animate-spin" /><Star size={40} className="text-white fill-white animate-spin" /><Star size={40} className="text-white fill-white animate-spin" /></div></div></div>)}
      <OverworldHUD stats={stats} multiplier={multiplier} onOpenJournal={() => setActiveGame('Journal')} onOpenAdmin={() => setActiveGame('Admin')} />
      <div className="bg-red-600 py-3 overflow-hidden whitespace-nowrap z-10 shrink-0 text-white text-[10px] uppercase font-black border-b-4 border-black"><div className="animate-marquee inline-block">{events.length > 0 ? events.map(e => `★ LIVE EVENT: ${e.title} IN ${e.worldId.toUpperCase()}! ${e.multiplier}X REWARDS! ★ `).join('     ') : '★ SUPER OWEN OVERWORLD ★ MISSION: MASTER EVERYTHING ★ WITTY & CURIOUS ★ '}</div></div>
      <header className="pt-16 pb-8 px-6 text-center z-10 shrink-0 text-black"><div className="relative inline-block text-black"><div className="absolute -inset-4 bg-black border-4 border-white translate-x-2 translate-y-2"></div><div className="relative bg-[#e76d42] border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-white font-bold"><h1 className="text-2xl md:text-4xl leading-tight uppercase tracking-tighter font-bold text-center text-white">SUPER <br /> OWEN WORLD</h1></div></div></header>
      <main className="max-w-7xl mx-auto px-6 z-10 flex-1 w-full text-center text-black mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-black mb-20 text-center">
      ...
      <footer className="mt-32 border-t-8 border-black mario-ground py-20 px-6 relative shrink-0 text-white text-center"><div className="max-w-4xl mx-auto text-center relative z-10 text-white text-center"><div className="w-24 h-40 mario-pipe mx-auto mb-8 text-center"></div><p className="text-white/50 text-[10px] uppercase tracking-widest italic font-bold text-center text-white">{VERSION}</p><button onClick={() => setActiveGame('Admin')} className="mt-8 text-[8px] uppercase text-white/20 hover:text-white font-bold text-center underline">Admin Console</button></div></footer>
    </div>
  );
}
